...
class Cond_Encoder():
    ...
    def __init__(self, condition_df: pd.DataFrame, categorical_columns: list, ordinal_columns: list, numeric_columns: list):
        self.columns_in_order = [col for col in condition_df.columns if col in categorical_columns + ordinal_columns + numeric_columns]
        ...
        self.units_per_column = [] 

        self.preprocess_conditions(condition_df)
    ...
    def preprocess_conditions(self, condition_df: pd.DataFrame):
        ...
        for column in self.columns_in_order:
            if column in self.categorical_columns:
                ...
                self.units_per_column.append(n_categories)  
                ...
                self.units_per_column.append(1)
                condition_dim += 1
        
        self.condition_dim = condition_dim  # Store the total condition dimensionality

    def encode(self, conditions: pd.DataFrame) -> torch.Tensor:
        """
    ...
    def inverse_transform(self, condition_data: torch.Tensor) -> pd.DataFrame:
        ...
        """
        df_sample = pd.DataFrame(condition_data.numpy(), columns=self.columns_in_order)
        ...
        for column in self.ordinal_columns + self.numeric_columns:
            scaler = self.min_max_scalers[column]
