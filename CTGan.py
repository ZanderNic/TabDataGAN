...
from .CTGan_utils import get_nolin_akt, get_loss_function, EarlyStopping
...
class CTGan(nn.Module):
    ...
    def __init__(
        ...
        classifier_nonlin_out: str = "sigmoid",
        ...
        self.classifier_nonlin_out = classifier_nonlin_out
        ...
        self.classifier_patient = classifier_patient
    ...
    def preprocess_and_load_data(self, ctgan_data_set, batch_size)-> DataLoader:
        conditiond_encoded = self.cond_encoder.encode(ctgan_data_set.dataframe())
        data_encoded = self.data_encoder.get_encoded_data()
    ...
    def train_classifier(self, dataset : CTGan_data_set, train_size:float = 0.8):
        ...
        num_train = int(train_size * dataset_size) 
        num_test = dataset_size - num_train

        train_dataset, test_dataset = random_split(dataset, [num_train, num_test])

        train_loader, test_loader = self.preprocess_and_load_data(train_dataset), self.preprocess_and_load_data(test_dataset)

        self.classifier.fit(train_loader=train_loader, test_loader=test_loader, lr=self.classifier_lr, opt_betas=self.dclassifier_opt_betas, patient=self.classifier_patient, loss=self.classifier_loss)
    ...
    def fit(self, ctgan_data_set: CTGan_data_set):
        ...
        if not self.generator and self.discriminator and self.classifier:
            ...
            self.output_space =  self.data_encoder.encoder_n_dim() 
            self.n_units_conditional =  self.cond_encoder #TODO
            ...
            self.classifier = Classifier(
                ...
                classifier_n_units_out = self.n_units_conditional,
                ...
                classifier_nonlin_out = self.classifier_nonlin_out,
